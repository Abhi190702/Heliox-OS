...
| Guarded fast path    | 27.534 ms median, 29.100 ms p95 across 100 non-LLM CPU-usage iterations; zero model calls     | This is not provider, browser, voice, camera, TTS, neural, or full-workflow latency            |
...
| Async responsiveness | 66 heartbeats during a real one-second CPU monitor; 20.511 ms maximum scheduler gap           | Windows timer granularity applies; this is not UI or hardware-input latency                    |
...
| Continuous voice       | Wake-word and follow-up listening, VAD endpoints, coordinated TTS, barge-in, and self-speech suppression                                                   |
