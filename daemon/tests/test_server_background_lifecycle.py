...
async def test_shutdown_quiesces_tasks_before_closing_their_owners() -> None:
    ...
    server._executor = SimpleNamespace(close=AsyncMock(side_effect=lambda: order.append("executor_closed")))
    ...
    assert order.index("background_stopped") < order.index("executor_closed")
